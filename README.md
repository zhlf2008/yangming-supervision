...
说明：`org-management.html` 仅作为旧链接兼容跳转到 `secretariat-org-management.html`，`semester-settings.html` 仅作为旧链接兼容跳转到 `secretariat-semesters.html?from=portal`；督察模块不再提供组织架构、人员管理和学期设置入口。新增登录账号、人员归属、职务和模块权限统一在 `secretariat-people.html` 处理，学期新增、编辑和当前学期切换统一在平台管理的 `secretariat-semesters.html` 处理。
